...
import type { FiltersDataProductList } from "../molecules/types";

interface ParamsProduct {
  searchQuery: string;
  setSearchQuery: (value: string) => void;
  initialDataFilters: FiltersDataProductList;
...
export default function ProductFilters({ searchQuery, setSearchQuery, initialDataFilters, filters, setFilters }: ParamsProduct) {
  return (
    <div className="mb-12 flex-1 bg-white p-4 rounded-xl">
      ...
      {
        initialDataFilters.map(data =>
          <GroupInputCheck info={data} filters={filters} setFilters={setFilters} />
        )
      }
